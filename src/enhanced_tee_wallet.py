...
class EnhancedTEEWallet:
    ...
    def __init__(self, wallet_id: str, policy: SigningPolicy = None, security_config: EnhancedSecurityConfig = None):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not success and (
                self.security_config.tee_fallback or self.security_config.backend_type in ["tee", "auto"]
            ):
    ...
    async def _auto_detect_hsm(self) -> Optional[HSMConfig]:
        ...
        test_configs = [
            ...
            create_hsm_config("pkcs11", library_path="/usr/lib/softhsm/libsofthsm2.so", slot_id=0, pin="1234"),
            # YubiHSM 2
            create_hsm_config("yubihsm", connector_url="http://localhost:12345", auth_key_id=1, password="password"),
            # Common PKCS#11 libraries
            create_hsm_config("pkcs11", library_path="/usr/lib/pkcs11/libCryptoki2_64.so"),
            create_hsm_config("pkcs11", library_path="/usr/lib/opensc-pkcs11.so"),
        ]
    ...
    async def secure_sign(self, tx_request: TransactionRequest, force_approve: bool = False) -> Dict[str, Any]:
        ...
        if not self._initialized:
            return {"success": False, "error": "Wallet not initialized", "backend": self.active_backend}
        ...
        try:
            ...
            if self.active_backend == "hsm":
                ...
                return {"success": False, "error": f"Invalid backend: {self.active_backend}"}
            ...
            return {"success": False, "error": str(e), "backend": self.active_backend}
        ...
        if self.active_backend == "tee" and self.tee_wallet:
            ...
            if self.hsm_manager:
                try:
                    ...
                    public_key_bytes = loop.run_until_complete(self.hsm_manager.get_wallet_public_key(self.wallet_id))
    ...
    def emergency_lock(self):
        ...
        self.policy = SigningPolicy(
            max_transaction_value=0, daily_limit=0, allowed_contracts=[], blocked_contracts=["*"]
        )
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        if self.active_backend == "hsm" and self.hsm_manager:
            ...
            status.update(
                {"hsm_audit_entries": len(hsm_audit), "hsm_security_info": self.hsm_manager.get_security_info()}
            )
...
async def create_enhanced_wallet(
    wallet_id: str, policy: SigningPolicy = None, prefer_hsm: bool = True, hsm_config: Optional[HSMConfig] = None
) -> EnhancedTEEWallet:
    """
    ...
    security_config = EnhancedSecurityConfig(
        backend_type="hsm" if prefer_hsm else "auto", hsm_config=hsm_config, tee_fallback=True, security_level="high"
    )
...
async def create_hsm_wallet(wallet_id: str, hsm_config: HSMConfig, policy: SigningPolicy = None) -> EnhancedTEEWallet:
    """Create wallet specifically using HSM backend"""
    security_config = EnhancedSecurityConfig(
        backend_type="hsm", hsm_config=hsm_config, tee_fallback=False, security_level="maximum"
    )
...
async def create_tee_wallet(wallet_id: str, policy: SigningPolicy = None) -> EnhancedTEEWallet:
    """Create wallet specifically using TEE backend"""
    security_config = EnhancedSecurityConfig(backend_type="tee", tee_fallback=False, security_level="high")
...
if __name__ == "__main__":
    ...
    async def demo_enhanced_wallet():
        ...
        try:
            ...
            # Test transaction
            tx = TransactionRequest(to_address="0x1234567890123456789012345678901234567890", value=500.0)
